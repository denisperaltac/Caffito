...
import { Department, Province } from "../../types/configuration";
...
const DepartmentsPage: React.FC = () => {
  ...
  const handleCreate = async () => {
    try {
      await configurationService.createDepartment(formData);
      setShowModal(false);
      setFormData({
        nombre: "",
        descripcion: "",
  ...
  const handleUpdate = async () => {
    ...
    try {
      await configurationService.updateDepartment(
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm("¿Está seguro de que desea eliminar este departamento?")
    ) {
      return;
    }

    try {
      await configurationService.deleteDepartment(id);
      loadData();
    } catch (err) {
  ...
  const handleEdit = (department: Department) => {
    ...
    setFormData({
      ...
      provinciaId: department.provinciaId,
  ...
  const getProvinceName = (provinciaId: string) => {
    const province = provinces.find((p) => p.id === provinciaId);
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {departments.map((department) => (
              <tr key={department.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {getProvinceName(department.provinciaId)}
                </td>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, provinciaId: e.target.value })
                    }
                    className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    required
                  >
